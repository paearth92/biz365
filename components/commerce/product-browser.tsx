...
export function ProductBrowser({ products, initialCategory }: { products: Product[]; initialCategory?: ProductCategory }) {
  ...
  return <div className="product-browser">
    ...
    <div className="browser-layout"><aside className="desktop-filters">{filters}</aside><div className="commerce-grid">{filtered.map(product => <ProductCard product={product} key={product.id} />)}{filtered.length===0 && <div className="empty-products"><SlidersHorizontal /><h3>No exact matches</h3><p>Clear a filter to see more Biz365 products.</p></div>}</div></div>

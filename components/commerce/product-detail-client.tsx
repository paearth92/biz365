...
import { Check, ChevronDown, Expand, Minus, Nfc, PackageCheck, Plus, RotateCcw, ShieldCheck, Smartphone, Truck, X } from "lucide-react";
...
function GalleryVisual({ product, view }: { product: Product; view: number }) {
  return (
    <div className={`gallery-scene gallery-scene--${view}`} aria-label={`${product.name} ${galleryLabels[view]}`} role="img">
...
export function ProductDetailClient({ product }: { product: Product }) {
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem("biz365-recent", JSON.stringify([product.slug, ...current.filter((x) => x !== product.slug)].slice(0, 4)));
    } catch {}
  }, [product.slug]);
  useEffect(() => {
    if (!lightbox) return;
  ...
  const estimate = (e: FormEvent) => {
    e.preventDefault();
    setDelivery(/^\d{5}$/.test(zip) ? "Estimated arrival: 4–7 business days" : "Enter a valid 5-digit U.S. ZIP code");
  };
  return (
    <>
      <div className="product-detail-grid phase3-detail">
        <div className="product-gallery">
          <button className="product-gallery__main" onClick={() => setLightbox(true)} aria-label={`Enlarge ${galleryLabels[view]} of ${product.name}`}>
          ...
          <div className="gallery-thumbs" role="tablist" aria-label="Product views">
            {galleryLabels.map((label, index) => (
              <button role="tab" aria-selected={view === index} className={view === index ? "active" : ""} onClick={() => setView(index)} key={label}>
                <Image className={`gallery-thumb-image gallery-thumb-image--${index}`} src={product.image} alt="" width={90} height={90} unoptimized />
                {label}
              </button>
            ))}
          </div>
        ...
        <section className="product-info-panel">
          ...
          {product.variants.length > 1 ? (
            <div className="variant-block">
              <div>
                <strong>{product.category === "review-stickers" ? "Choose pack size" : "Choose an option"}</strong>
                <span>{variant.name}</span>
              </div>
              <div className="variant-options">
                {product.variants.map((item) => (
                  <button aria-pressed={variantId === item.id} className={variantId === item.id ? "active" : ""} onClick={() => setVariantId(item.id)} key={item.id}>
                    {item.name}
                  </button>
                ))}
              </div>
          ...
          <form className="delivery-estimator" onSubmit={estimate}>
            ...
            <input inputMode="numeric" maxLength={5} value={zip} onChange={(e) => setZip(e.target.value.replace(/\D/g, ""))} placeholder="ZIP code" aria-label="U.S. ZIP code" />
            <button>Check</button>
      ...
      {lightbox && (
        <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label={`${product.name} enlarged image`}>
          ...
          <div className="lightbox-nav">
            {galleryLabels.map((label, index) => (
              <button className={view === index ? "active" : ""} onClick={() => setView(index)} key={label}>
                {label}
              </button>
            ))}
          </div>
    ...
    <>
      <section className="product-story">
        <div>
          ...
          <p>Both paths open the same intended business destination. Customers choose whichever feels natural—without downloading a separate app.</p>
          <ul>
      ...
      <section className="specification-section">
        <div>
          ...
          <p>Final production measurements remain independently replaceable when physical inventory is confirmed.</p>
        </div>
        <dl>
          {[...Object.entries(product.specifications), ["Dimensions", experience.dimensions], ["Material", experience.material], ["Included", experience.included]].map(([key, value]) => (
            <div key={key}>
              <dt>{key}</dt>
              <dd>{value}</dd>
            </div>
